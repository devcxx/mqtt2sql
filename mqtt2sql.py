...
def write2sql(message):
    ...
    try:
        if args.sqltype=='mysql':
            db = MySQLdb.connect(host=args.sqlhost,port=args.sqlport, user=args.sqlusername, passwd=args.sqlpassword, db=args.sqldb)
    ...
    while transactionretry>0:
        ...
        try:
            ...
            if args.sqltype=='mysql':
                cursor.execute("INSERT INTO {0} (id,ts,topic,value,qos,retain,active) values(default,'{1}','{2}','{3}','{4}',{5},{6})".format(args.sqltable, ts, message.topic, message.payload, message.qos, message.retain, active))
            elif args.sqltype=='sqlite':
                # strtime=str(time.strftime("%Y-%m-%d %H:%M:%S"))
